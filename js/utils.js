...
(function (CPManager) {
	CPManager.utils = {
			...
			if (!CPManager.state.zoneColors[zoneId]) {
				CPManager.state.zoneColors[zoneId] =
					CPManager.config.predefinedColors[
						CPManager.state.zoneColorIndex % CPManager.config.predefinedColors.length
					];
				CPManager.state.zoneColorIndex =
					(CPManager.state.zoneColorIndex + 1) % CPManager.config.predefinedColors.length;
			}
		...
		getAuthViaColor: function (authMethod) {
			if (!CPManager.state.authViaColors[authMethod]) {
				CPManager.state.authViaColors[authMethod] =
					CPManager.config.predefinedColors[
						CPManager.state.authViaColorIndex % CPManager.config.predefinedColors.length
					];
				CPManager.state.authViaColorIndex =
					(CPManager.state.authViaColorIndex + 1) % CPManager.config.predefinedColors.length;
				// Simple attempt to avoid direct overlap with zoneColorIndex if they happen to align
				if (CPManager.state.authViaColorIndex === CPManager.state.zoneColorIndex) {
					CPManager.state.authViaColorIndex =
						(CPManager.state.authViaColorIndex + 1) % CPManager.config.predefinedColors.length;
				}
			}
		...
		formatAuthVia: function (authVia) {
			return CPManager.config.authViaMapping[authVia] || authVia || CPManager.config.placeholderValue;
		},
		...
		formatVoucherTimestamp: function (timestamp) {
			if (!timestamp || timestamp === 0) return CPManager.config.placeholderValue;
			try {
