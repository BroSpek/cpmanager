...
async function fetchAllZoneData(forceRefresh = false) {
	...
	try {
		...
		if (allConfiguredZones.length === 0 && !(data && Array.isArray(data.rows))) {
 ...
 */
async function loadZoneInfo(forceRefreshDetails = false) {
	// Added forceRefreshDetails
		...
		try {
			...
			if (zoneDetailsResponse && zoneDetailsResponse.zone) {
				...
				for (const [key, value] of Object.entries(zoneData)) {
					...
					const readableKey =
						zoneFieldMappings[key] || key.charAt(0).toUpperCase() + key.slice(1).replace(/([A-Z])/g, " $1");
					let displayValue;
...
async function saveZoneSettings() {
	...
	try {
		...
		if (result && result.result === "saved") {
			showToast(
				`Zone settings updated for "${zoneSettingsActuallyEdited.description || uuid.substring(0, 8)}".`,
				"success"
			);
