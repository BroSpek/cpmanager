	...
	CPManager.notifications = {
		requestNotificationPermission: async function () {
			if (!("Notification" in window)) {
				CPManager.ui.showToast("Browser does not support desktop notification", "error");
				return;
			}
			...
			if (permission === "granted") {
				...
				if (CPManager.state.currentApiKey && CPManager.state.currentApiSecret) {
					...
					CPManager.ui.showToast("API credentials not set. Cannot start polling.", "warning");
					CPManager.notifications.updateNotificationToggleState(false, false);
				}
			} else {
				CPManager.ui.showToast("Sign-in notifications permission denied.", "warning");
		...
		updateNotificationToggleState: function (isEnabled, isErrorState = false) {
			...
			if (toggleButton) {
				if (isErrorState) {
					toggleButton.innerHTML = '<i class="fas fa-exclamation-triangle text-red-500"></i>';
					toggleButton.setAttribute("aria-label", "Notifications disabled (errors). Click to re-try.");
		...
		handleVisibilityChange: function () {
			if (
				...
				localStorage.getItem(CPManager.config.localStorageKeys.signInNotificationsEnabled) !== "true"
			) {
				return;
			}
			if (CPManager.state.notifications.sessionPollIntervalId) {
		...
		initializeNotifications: function () {
			const storedPreference = localStorage.getItem(CPManager.config.localStorageKeys.signInNotificationsEnabled);
