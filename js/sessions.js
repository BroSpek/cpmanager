...
async function fetchManagerSessionStatus() {
	...
	try {
		...
		const response = await fetch(
			`${OPNsenseConfig.baseUrl}/api/captiveportal/access/status/${zoneIdForStatusCall}`
		);
		...
		if (data && data.sessionId && data.clientState === "AUTHORIZED") {
			...
			console.log(
				...
				managerSessionDetails.ipAddress,
				"ID:",
				managerSessionDetails.sessionId.substring(0, 8) + "..."
			);
	...
	sessions.forEach((session) => {
		...
		if (isManagerCurrentDeviceSession) {
			managerIconHtml = `<span class="info-tag bg-blue-600 text-white flex items-center" title="This is your current device's session (IP: ${
				managerSessionDetails.ipAddress || "N/A"
...
async function handleDisconnectAllSessions() {
	...
	const selectedZoneName = selectedZoneId
		? sessionZoneFilterSelect.options[sessionZoneFilterSelect.selectedIndex].text || `Zone ${selectedZoneId}`
		: "All Zones";
				...
				callApi(`/session/disconnect/${session.zoneid}`, "POST", payload)
					.then((result) => {
						if (
							...
							failureCount++;
							console.warn(
								`Failed to disconnect session ${session.sessionId} (IP: ${session.ipAddress}): ${
...
function initializeSessionEventListeners() {
	...
	if (sessionCardContainer) {
		sessionCardContainer.addEventListener("click", (e) => {
			...
			if (disconnectButton) {
				...
				const isMy =
					managerSessionDetails &&
					managerSessionDetails.sessionId === sessionId &&
					managerSessionDetails.zoneid === zoneid;
	...
	if (findMySessionBtn) {
		findMySessionBtn.addEventListener("click", async () => {
			// Made async
			if (managerSessionDetails && managerSessionDetails.sessionId) {
				showToast(
					`Found your session (IP: ${managerSessionDetails.ipAddress}). Highlighting and filtering...`,
					"info"
				);
				...
				setTimeout(() => {
					const highlightedCard = sessionCardContainer
						? sessionCardContainer.querySelector(".ring-blue-500")
						: null;
					if (highlightedCard) {
