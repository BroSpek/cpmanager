	...
	CPManager.api = {
		...
		callApi: async function (endpoint, method = "GET", body = null) {
			...
			if (!CPManager.config.baseUrl) {
				...
				CPManager.ui.showToast("API Base URL not set. Please configure in settings.", "error", 8000);
			...
			if (CPManager.state.currentApiKey && CPManager.state.currentApiSecret) {
				headers.append(
					"Authorization",
					"Basic " + btoa(CPManager.state.currentApiKey + ":" + CPManager.state.currentApiSecret)
				);
			} else {
				const errorMessage = "API Key or Secret is missing. Cannot make API call.";
				console.error(errorMessage, "Endpoint:", endpoint);
				CPManager.ui.showToast("API Key or Secret not set. Please configure in settings.", "error");
			...
			try {
				...
				if (!response.ok) {
					...
					CPManager.ui.showToast(`API Error: ${errorData.message} ${errorData.detail}`, "error");
				...
				try {
					...
					console.warn(
						`API Warning for ${method} ${url}: Successful response was not valid JSON and not recognized text. Content:`,
