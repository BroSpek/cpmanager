 ...
 */
async function loadVoucherProviders(forceRefresh = false) { // Added forceRefresh
    ...
    try {
        ...
        if (providers && Array.isArray(providers)) {
            ...
            } else if (!providers) { // Covers null or undefined response
                showToast("Failed to fetch voucher providers. Endpoint might not exist or no providers configured.", "warning");
            }
        }
    } catch (error) {
...
function populateVoucherProviderSelect(providers) {
    ...
    if (providers.length === 1 && !providerToSelect) { // Auto-select if only one provider AND no saved preference
    ...
    } else if (providerToSelect) { // Restore last selected provider
 ...
 */
function handleProviderSelection(providerId, forceRefreshGroups = false) { // Added forceRefreshGroups
    if (providerId) {
 ...
 * @param {boolean} [forceRefresh=false] - If true, forces a re-fetch even if data exists.
 */
async function loadVoucherGroups(providerId, forceRefresh = false) { // Added forceRefresh
    ...
    if(voucherCardContainer && (!cachedVouchersData[`${providerId}_${voucherGroupSelect.value}`] || forceRefresh)) { // only show skeleton if vouchers also need loading
    ...
    try {
        ...
        if (groups && Array.isArray(groups)) {
            ...
            console.error(`Error loading voucher groups for provider ${providerId}: API response is not an array or is undefined.`, groups);
            voucherGroupSelect.innerHTML = '<option value="">Error: Unexpected data for groups.</option>';
            if(voucherCardContainer) showNoDataMessage(voucherCardContainer, "Error loading voucher groups.", "fas fa-exclamation-triangle");
 ...
 * @param {boolean} [forceRefresh=false] - If true, forces a re-fetch even if data exists.
 */
async function loadVouchersForGroup(providerId, groupName, forceRefresh = false) { // Added forceRefresh
    if (!providerId || !groupName || !voucherCardContainer) {
        if(voucherCardContainer) showNoDataMessage(voucherCardContainer, "Provider or group not selected.", "fas fa-info-circle");
        currentVouchers = [];
    ...
    try {
        ...
        if (vouchers && Array.isArray(vouchers)) {
            ...
            currentVouchers = [];
            renderVouchers([], groupName); // Clear display
            console.error(`Error loading vouchers for group ${groupName} (Provider: ${providerId}): API response is not an array or is undefined.`, vouchers);
            showNoDataMessage(voucherCardContainer, "Error: Unexpected data for vouchers.", "fas fa-exclamation-triangle");
        }
    } catch (error) {
...
function renderVouchers(vouchers, groupName) {
    ...
    vouchers.forEach((voucher) => {
        ...
        card.className = 'voucher-card bg-gray-50 p-3 rounded-lg shadow border border-gray-200 relative';
        ...
        card.innerHTML = `
            ...
            <div class="card-summary cursor-pointer pb-1" role="button" tabindex="0" aria-expanded="false" aria-controls="voucher-details-${voucher.username}">
                 <div class="info-row"><span class="info-label">Voucher Code</span><span class="info-value summary-main-value">${voucher.username}</span></div>
            </div>
...
function openGenerateVoucherModal() {
    ...
    if(voucherCountCustom) { voucherCountCustom.classList.add('hidden'); voucherCountCustom.value = '1'; }
    if(voucherLifetimeSelect) voucherLifetimeSelect.value = '240'; // Default lifetime (4 hours in minutes)
    if(voucherLifetimeCustom) { voucherLifetimeCustom.classList.add('hidden'); voucherLifetimeCustom.value = ''; }
    if(voucherUsageSelect) voucherUsageSelect.value = '0'; // Default usage (Never expires)
    if(voucherUsageCustom) { voucherUsageCustom.classList.add('hidden'); voucherUsageCustom.value = ''; }
...
async function handleSubmitGenerateVoucher() {
    ...
    let count = voucherCountSelect.value === 'custom' ? parseInt(voucherCountCustom.value) : parseInt(voucherCountSelect.value);
    if (isNaN(count) || count < 1) { showToast('Number of vouchers must be at least 1.', 'error'); return; }

    let lifetimeInSeconds;
    if (voucherLifetimeSelect.value === 'custom') {
        const customMinutes = parseInt(voucherLifetimeCustom.value);
        if (isNaN(customMinutes) || customMinutes < 1) { showToast('Custom Validity (minutes) must be at least 1.', 'error'); return; }
    ...
    if (voucherUsageSelect.value === 'custom') {
        ...
        if (isNaN(customHours) || customHours < 0) { showToast('Custom Expires In (hours) must be a non-negative number.', 'error'); return; }
        usageInSeconds = customHours * 3600;
    } else {
        const selectedUsageHours = parseInt(voucherUsageSelect.value);
        if (isNaN(selectedUsageHours)) { showToast('Invalid Expires In selection.', 'error'); return; }
    ...
    try {
        ...
        if (result && Array.isArray(result) && result.length > 0 && result[0].username) { // Check if result is an array of vouchers
            lastGeneratedVouchers = result;
            showToast(`Vouchers generated successfully in group "${groupname}" for provider "${selectedProvider}". CSV downloading...`, 'success');
        ...
        } else if (result && result.status === 'ok_text' && result.message && result.message.toLowerCase().includes('created')) {
            showToast(`Vouchers generated for group "${groupname}". (API Text OK). No CSV data from this response.`, 'success');
            ...
            showToast(`Failed to generate vouchers: ${result.message || (result.status === 'error' ? 'API reported an error.' : 'Unknown error or no vouchers returned.')}`, 'error');
            lastGeneratedVouchers = [];
        }
    } catch (error) {
        lastGeneratedVouchers = [];
...
async function handleRevokeVoucher(providerId, voucherCode, groupName) {
    ...
    showConfirmationModal('Revoke Voucher?', `Are you sure you want to revoke voucher <strong>${voucherCode}</strong> for provider "<strong>${providerId}</strong>"? This action marks it as expired and cannot be undone.`, async () => {
        try {
            const result = await callApi(`/voucher/expire_voucher/${providerId}`, 'POST', { username: voucherCode });
            if (result && result.status !== 'error') {
                showToast(`Voucher ${voucherCode} processed for expiration.`, 'success');
            } else if (result && result.status === 'error') {
                showToast(`Voucher ${voucherCode} could not be expired. API: ${result.message || 'Error'}`, 'warning');
            } else {
                showToast(`Voucher ${voucherCode} expiration processed with unknown API response. Status: ${result.status || 'N/A'}.`, 'info');
            }
...
async function handleDropExpiredVouchers() {
    ...
    if (!selectedProvider) { showToast('Please select a voucher provider first.', 'error'); return; }
    if (!selectedGroup) { showToast('Please select a voucher group first.', 'error'); return; }

    showConfirmationModal('Drop Expired Vouchers?', `Are you sure you want to remove all expired vouchers from group "<strong>${selectedGroup}</strong>" for provider "<strong>${selectedProvider}</strong>"?`, async () => {
        try {
            const result = await callApi(`/voucher/drop_expired_vouchers/${selectedProvider}/${selectedGroup}`, 'POST', {});
            if (result && result.status === 'drop') {
                showToast(`Expired vouchers from group "${selectedGroup}" (Provider: ${selectedProvider}) cleaned up. Count: ${result.count || 0}`, 'success');
            } else {
                showToast(`Cleanup for group "${selectedGroup}" (Provider: ${selectedProvider}) processed. Status: ${result.status || 'unknown'}. Count: ${result.count || 0}`, 'info');
            }
...
async function handleDropVoucherGroup() {
    ...
    const selectedProvider = voucherProviderSelect.value;
    const selectedGroup = voucherGroupSelect.value;
    if (!selectedProvider) { showToast('Please select a voucher provider first.', 'warning'); return; }
    if (!selectedGroup) { showToast('Please select a voucher group to delete.', 'warning'); return; }

    showConfirmationModal('Drop Voucher Group?', `Are you sure you want to delete the entire voucher group "<strong>${selectedGroup}</strong>" for provider "<strong>${selectedProvider}</strong>"? This will delete all vouchers within it and cannot be undone.`, async () => {
        try {
            const result = await callApi(`/voucher/drop_voucher_group/${selectedProvider}/${selectedGroup}`, 'POST', {});
            if (result && result.status !== 'error') {
                showToast(`Voucher group "${selectedGroup}" (Provider: ${selectedProvider}) deleted successfully.`, 'success');
                // Clear relevant caches
                if (cachedVoucherGroups[selectedProvider]) {
                    cachedVoucherGroups[selectedProvider] = cachedVoucherGroups[selectedProvider].filter(g => g !== selectedGroup);
                }
                ...
                showToast(`Failed to delete voucher group "${selectedGroup}". API: ${result ? result.message : 'Unknown error.'}`, 'error');
            }
        } catch (error) {
            // Error toast handled by callApi
        }
...
function initializeVoucherEventListeners() {
    ...
    if (voucherGroupSelect) {
        voucherGroupSelect.addEventListener('change', async (e) => { // Made async
