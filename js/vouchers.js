  ...
  CPManager.vouchers = {
    ...
    renderVouchers: function (vouchersToRender, groupName) {
      ...
      paginatedVouchers.forEach((voucher) => {
        ...
        card.innerHTML = `<div class="flex justify-between items-center mb-1">${checkboxHTML}<div class="flex items-center"><span class="py-0.5 px-1.5 text-xs rounded-sm text-white max-w-[100px] whitespace-nowrap overflow-hidden text-ellipsis ${stateTagColor}" title="State: ${voucher.state}">${voucher.state}</span></div></div><div id="${cardSummaryId}" class="voucher-summary cursor-pointer pb-1" role="button" tabindex="0" aria-expanded="false"><div class="flex justify-between items-start py-1"><span class="font-semibold text-sm mr-3 whitespace-nowrap flex-shrink-0 text-slate-500 dark:text-slate-400">Voucher Code</span><span class="text-sm text-right font-semibold break-all flex-grow text-slate-800 dark:text-slate-200">${voucher.username}</span></div></div><div class="card-details-content max-h-0 overflow-hidden transition-all duration-300 ease-out text-sm space-y-1" id="${cardDetailsId}" aria-hidden="true"><div class="flex justify-between items-start py-1"><span class="font-semibold text-sm mr-3 whitespace-nowrap flex-shrink-0 text-slate-500 dark:text-slate-400">Validity</span> <span class="text-sm text-right break-all flex-grow text-slate-600 dark:text-slate-300">${CPManager.utils.formatDuration(voucher.validity, "seconds")}</span></div><div class="flex justify-between items-start py-1"><span class="font-semibold text-sm mr-3 whitespace-nowrap flex-shrink-0 text-slate-500 dark:text-slate-400">Start Time</span> <span class="text-sm text-right break-all flex-grow text-slate-600 dark:text-slate-300">${CPManager.utils.formatVoucherTimestamp(voucher.starttime)}</span></div><div class="flex justify-between items-start py-1"><span class="font-semibold text-sm mr-3 whitespace-nowrap flex-shrink-0 text-slate-500 dark:text-slate-400">End Time</span> <span class="text-sm text-right break-all flex-grow text-slate-600 dark:text-slate-300">${CPManager.utils.formatVoucherTimestamp(voucher.endtime)}</span></div><div class="flex justify-between items-start py-1"><span class="font-semibold text-sm mr-3 whitespace-nowrap flex-shrink-0 text-slate-500 dark:text-slate-400">Expires At</span><span class="text-sm text-right break-all flex-grow text-slate-600 dark:text-slate-300">${voucher.expirytime && voucher.expirytime !== 0 ? CPManager.utils.formatVoucherTimestamp(voucher.expirytime) : "Never"}</span></div></div>`;
