...
async function loadDashboardData(forceRefresh = false) {
	...
	if (
		...
		!uploadLegendValue ||
		!downloadLegendValue ||
		!uploadPercentageSpan ||
		!downloadPercentageSpan
	) {
		console.error("One or more dashboard elements are missing from the DOM.");
		if (dashboardStatsContainer)
			dashboardStatsContainer.innerHTML = `<p class="text-red-500 col-span-full text-center">Error: Dashboard UI elements missing.</p>`;
		return;
	}
	...
	try {
		...
		if (allConfiguredZones.length === 0 && totalZonesCount === 0) {
		...
		document
			...
			?.addEventListener("click", () => setActiveTab("sessions"));
		document
			.getElementById("dashboard-configured-zones-card")
			?.addEventListener("click", () => setActiveTab("info"));
		document.getElementById("dashboard-vouchers-card")?.addEventListener("click", () => setActiveTab("vouchers"));
		["dashboard-active-sessions-card", "dashboard-configured-zones-card", "dashboard-vouchers-card"].forEach(
			(id) => {
		...
		if (
			forceRefresh ||
			totalClientUploadBytes !== originalUploadBytes ||
			totalClientDownloadBytes !== originalDownloadBytes ||
			!dataUsageChart
		) {
			...
			if (donutTotalData) {
				donutTotalData.innerHTML = `
                    <span style="font-weight: bold; display: block; font-size: 1.2em;">${formatBytes(
						currentTotalData
					)}</span>
			...
			if (uploadPercentageSpan)
				uploadPercentageSpan.textContent = `(${(currentTotalData > 0
					? (totalClientUploadBytes / currentTotalData) * 100
					: 0
				).toFixed(1)}%)`;
			if (downloadPercentageSpan)
				downloadPercentageSpan.textContent = `(${(currentTotalData > 0
		...
		if (dashboardStatsContainer)
			dashboardStatsContainer.innerHTML = `<p class="text-red-500 col-span-full text-center">Error loading dashboard data. Check console.</p>`;
	...
	legendItems.forEach((item) => {
		item.addEventListener("mouseenter", () => {
			...
			if (segmentValue !== undefined && donutTotalData) {
				...
				donutTotalData.innerHTML = `
                    <span style="color: ${segmentColor}; font-weight: bold; display: block; font-size: 1.2em;">${formatBytes(
					segmentValue
				)}</span>
