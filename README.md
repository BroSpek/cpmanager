...
<details><summary>If you encounter "Failed to fetch" errors or "Cannot reach OPNsense API" messages when using the application, especially if you are hosting it on a different domain, subdomain, IP address, or port than your OPNsense firewall's web interface, you are likely experiencing a **Cross-Origin Resource Sharing (CORS)** issue.</summary>
...
<details><summary>The Captive Portal Manager allows you to easily generate new vouchers for guest access through your OPNsense Captive Portal.</summary>
...
<details><summary>The Captive Portal Manager supports local notifications for new user sign-ins on your configured captive portal zones.</summary>
...
-   **Weaknesses**:
    ...
    -   **Not Real-Time:** Notifications rely on periodic checks (polling), so there's a slight delay.
    -   **Browser/App Must Run:** Your browser (with the app or its background service) needs to be active.
    -   **Needs Network:** The device requires an internet/network connection to check for sign-ins.
    -   **Permission Needed:** Notifications won't work if you don't grant permission in your browser.
    -   **API Errors Can Stop Them:** If the app can't connect to OPNsense repeatedly, notifications will temporarily stop.
    -   **Device Specific:** Notifications are tied to the specific browser and device where you enable them.
